package open3d.objects
{
	...
	public class Mesh extends Object3D
	{
		...
		private var _vertexIndexes:Array;
		...
		private var nVec:Vector.<Number>;
		...
		protected function buildFaces(material:Material):void
		{
			...
			for (var i:int = 0; i < _indices_length; ++i)
			{
				...
				var vertex:Vector.<Number> = Vector.<Number>([3 * i0 + 2, 3 * i1 + 2, 3 * i2 + 2, 0]);
				vertex.fixed = true;
				var _face:Face = _faces[i] = new Face(this, index, 3 * i0 + 2, 3 * i1 + 2, 3 * i2 + 2);
				
				// register face index for z-sort
				_faceIndexes[i] = index;
				_vertexIndexes[i] = vertex;
				
				nVec.push(3 * i0 + 2, 3 * i1 + 2, 3 * i2 + 2, 0);
...
var i:int = 0;
				/*
				var k:int = -1;
				for (i = 0; i < _faceIndexes_length; ++i)
				{
					k+=4;
					_faceIndexes[i].w = nVec[k];
				}
				*/
				// 0, 1, 2, 3
				// 4, 5, 6, 7
				...
				/*
				var i:int = -1;
				for each(var index:Vector.<Number> in _faceIndexes)
				{
					_triangles_indices[int(++i)] = index[0];
					_triangles_indices[int(++i)] = index[1];
					_triangles_indices[int(++i)] = index[2];
				}
				*/
				var j:int = -1;
				for(i=0;i<_faceIndexes_length;++i)
				{
					var index:Vector3D = _faceIndexes[i];
					_triangles_indices[int(++j)] = index.x;
					_triangles_indices[int(++j)] = index.y;
					_triangles_indices[int(++j)] = index.z;
				}
			}
