package
{
	...
	public class reader extends BasicTemplate
	{
		...
		{
			//setting
			FLARConfig.USE_DEDUG = true;
			FLARConfig.ALWAYS_RESET = false;
			FLARConfig.USE_CONTEXT = true;
			FLARConfig.USE_CAMERA = false;
		...
		override protected function onInit():void
		{
			...
			// get model data
			LoaderUtil.loadXML(FLARConfig.MODEL_DATA_URL, onGetModelData);
		}
		
		public static function getModelNameByCode(target:String):String
		{
			if(FLARConfig.MODEL_XML)
				return FLARConfig.MODEL_XML.ingredient.(code == target).name.text();
			else{
		...
		{
			if(FLARConfig.MODEL_XML)
				return FLARConfig.MODEL_XML.ingredient.(code == target).src.text();
			else{
				trace("[ERROR] Must load xml data first!");
				return null;
			}
		...
		private function onGetModelData(event:Event):void
		{
			...
			FLARConfig.MODEL_XML = event.target["data"];
			
			//tell all
			FLARConfig.dispatchEvent(new Event(Event.COMPLETE));
			
			initARQR();
			
			// cam as default
			if(FLARConfig.USE_CAMERA)
				toggleCamera();
		}
		
		private var _bmpData:BitmapData = new BitmapData(CANVAS_HEIGHT, CANVAS_HEIGHT, false, 0);
		...
		private function initARQR():void
		{
			...
			var _bitmapData:BitmapData = new BitmapData(QR_SIZE+100, QR_SIZE+100, false, 0xFFFFFF);
			_bitmapData.draw(new ImageData, new Matrix(1, 0, 0, 1, _bitmapData.width/2 - QR_SIZE/2, _bitmapData.height/2 - QR_SIZE/2));
			setBitmap(new Bitmap(_bitmapData));

			// menu
			if(FLARConfig.USE_CONTEXT)
			{
				SystemUtil.addContext(this, "ARCode version 0.1");
			...
			_modelViewer.load("assets/cat.dae");
		}
		...
		private function onQRCodeComplete(event:Event):void
		{
			...
			// dispatch
			FLARConfig.setCode(QRManager.result);
		}
		...
		override protected function onPreRender():void
		{
			...
			/*
			if(_isQRDecoded || FLARConfig.ALWAYS_RESET)
			{
		...
		private function process():void
		{
			...
			}else if(n<=FLARConfig.AR_RESET_NUM && _isQRDecoded){
				reset();
			}else{
				if(QRManager.result!="")
				{
