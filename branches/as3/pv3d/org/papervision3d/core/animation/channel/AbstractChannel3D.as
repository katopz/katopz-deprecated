package org.papervision3d.core.animation.channel
{
	...
	{	
		...
		{	
			...
			var _length:int = this.keyFrames.length;
			
			currentIndex = keyframe;
			currentIndex = currentIndex < _length - 1 ? currentIndex : 0;
			
			nextIndex = currentIndex + 1;
			
			currentKeyFrame = this.keyFrames[currentIndex];
			nextKeyFrame = nextIndex < _length ? this.keyFrames[nextIndex] : null;
		...
		{	
			var _length:int = this.keyFrames.length;
			currentIndex = Math.floor((_length-1) * time);
			currentIndex = currentIndex < _length - 1 ? currentIndex : 0;
			nextIndex = currentIndex + 1;
