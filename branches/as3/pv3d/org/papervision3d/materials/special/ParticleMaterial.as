	...
	public class ParticleMaterial extends MaterialObject3D implements IParticleDrawer
	{
		...
		public function drawParticle(particle:Particle, graphics:Graphics, renderSessionData:RenderSessionData):void
		{
			...
			if(shape == SHAPE_SQUARE){
				...
				graphics.drawCircle(renderrect.x+renderrect.width*.5, renderrect.y+renderrect.width*.5, renderrect.width*.5);
			}else{
		...
		public function updateRenderRect(particle : Particle) :void
		{
			...
			renderrect.x = particle.vertex3D.vertex3DInstance.x - (renderrect.width*.5); 
			renderrect.y = particle.vertex3D.vertex3DInstance.y - (renderrect.width*.5);
