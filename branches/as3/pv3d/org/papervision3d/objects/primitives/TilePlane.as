package org.papervision3d.objects.primitives {
	...
	*/
	public class TilePlane extends TriangleMesh3D
	{
		/**
		...
		*/
		public function TilePlane( materials:MaterialsList, width:Number=0, height:Number=0, segmentsW:Number=0, segmentsH:Number=0 )
		{
			...
			buildPlane( "all", width, height );
			
		}
	
		private function buildPlane( mat:String,  width:Number, height:Number ):void
		{
	...
	/*
			...
			{
				for( var iy:int = gridY1; iy > 0; iy-- )
				{
					var x :Number = ix * iW - textureX;
					var y :Number = iy * iH - textureY - iH;
	
					vertices.push( new Vertex3D( x, 0, y ) );
				}
			}
			*/
			for( var ix:int = 0; ix < gridX1; ix++ )
			...
			for(  ix = 0; ix < gridX; ix++ )
			{
				...
				{
					matInstance = materials.getMaterialByName(ix+"_"+(gridY-iy-1))
					
					if(!matInstance){
						matInstance=materials.getMaterialByName( "all" )
					}
