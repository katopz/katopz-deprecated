		...
		*/
		...
		private function buildCylinder( fRadius:Number, fHeight:Number, fTopRadius:Number, fTopFace:Boolean, fBottomFace:Boolean ):void
		{
			...
			for (j=0;j<(iVer+1);j++) { // vertical
				...
				var fZ:Number = fHeight*(j/(iVer+0))-fHeight*.5;//-fRadius*Math.cos(fRad1*Math.PI);
			...
			for (j=0;j<iVerNum;j++) {
				...
				if (j==0||j==(iVerNum-1)) {
					for (i=0;i<(iHorNum-2);i++) {
						// uv
						var iI:int = Math.floor(i*.5);
						aP1 = aVtc[j][iI];
						aP2 = (i%2==0)? (aVtc[j][iHorNum-2-iI]) : (aVtc[j][iI+1]);
						aP3 = (i%2==0)? (aVtc[j][iHorNum-1-iI]) : (aVtc[j][iHorNum-2-iI]);
	
						var bTop:Boolean = j==0;
						aP1uv = new NumberUV( (bTop?1:0)+(bTop?-1:1)*(aP1.x/fRadius*.5+.5), aP1.z/fRadius*.5+.5 );
						aP2uv = new NumberUV( (bTop?1:0)+(bTop?-1:1)*(aP2.x/fRadius*.5+.5), aP2.z/fRadius*.5+.5 );
						aP3uv = new NumberUV( (bTop?1:0)+(bTop?-1:1)*(aP3.x/fRadius*.5+.5), aP3.z/fRadius*.5+.5 );
