package  
{
	 ...
	 */
	public class ColadaTest  extends Sprite 
	{
		...
		private var trianglesperDraw : uint = 1000
		...
		private function urlComplete(event : Event) : void
		{
			...
			triangles = colladaParser.triangles
		...
		{ 
			...
			if (stateCount == 2)
			{
				var num : int = trianglesperDraw + drawcount
				...
				for (var i : Number = drawcount;i < num; i++)
				{
					...
					var material : Material = materials[t.mat]
					if (material == null)
					{
						bmdmat = defaultBmd
					}
		...
		private function draw(triangle_arr : Array,mat : String) : void
		{
			...
			var bmdmat : BitmapData
			var material : Material = materials[mat]
			if (material == null)
			{
				bmdmat = defaultBmd
			}
		...
		private function sortTriangles() : void
		{
			...
			triangles.sortOn("screenZ", Array.NUMERIC)//| Array.DESCENDING);
		}
