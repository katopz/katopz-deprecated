package  
{
	 ...
	 * @mod katopz
	 */
	public final class ScaleCopyPixelParticles extends Sprite 
	{
		private const MAX_PARTICLES: int = 1000;//0x400;
		...
		private var _focus:Number = 1000;
		private var _zoom:Number = 1;
		private var fz:Number;
		private var _bmp_graphics:Graphics; 
		
		public function ScaleCopyPixelParticles()
		{
			addChild(bmp);
			screenSetup();
			mouseEnabled = false;
		...
		private function screenSetup(): void
		{
			...
			///perspectiveProjection.focalLength+=5000
		...
		private function onEnterFrame( event: Event ): void
		{
			...
			*/
			...
			do
			{
				...
				if( minZ < pz )
				{
					...
					*/
					if(int(particle.screenX) != int(xi) &&
					int(particle.screenY) != int(yi) &&
					int(particle.screenZ) != int(pz))
					{
					particle.screenX = xi;
					particle.screenY = yi;
					particle.screenZ = pz;
					...
					var scale:Number = fz / (fz + pz);
					
					scale = int(scale*25)/25;
					 
					//if(scale!=particle.scale)
					{
						...
						var tmp_bmp:BitmapData = mats[(index+j)%29];
						
						particle.bitmapData = new BitmapData(size*scale, size*scale,true,0x00000000)
						
						particle.bitmapData.draw(tmp_bmp,matrix,null,null,null,true);
					}
					
					//particle.bitmapData = mats[(index+j)%29];
			...
			*/
			...
			//render
			//_bmp_graphics.clear();
			var _bmp_bitmapData:BitmapData = bmp.bitmapData = new BitmapData(800,600,true,0x000000); 
			
			_bmp_bitmapData.lock();
			...
			for each(var _particle:Particle in particles)
			{
				...
				//xi = cx+_particle.screenX;
				//yi = cy+_particle.screenY;
				...
				//var bmp2:Bitmap = new Bitmap(new BitmapData(size, size));
				//var tmp_bmp:BitmapData = bmp2.bitmapData; 
				var tmp_bmp:BitmapData = _particle.bitmapData;//mats[(_particle.index+j)%29];
				
				//bmp2.z = _particle.screenX;  

				_bmp_bitmapData.copyPixels(tmp_bmp, tmp_bmp.rect, new Point(_particle.screenX, _particle.screenY), null, null, true);
			}
			_bmp_bitmapData.unlock();
