	...
	public class Board extends SDSprite
	{
		...
		public function shuffle(e:* = null):void
		{
			...
			*/
			...
			if(Rule.checkCol(_crystals) || Rule.checkRow(_crystals))
				shuffle();
		}
			...
			if (_crystal)
			{
				switch (_status)
				{
					...
					case SELECT_SWAP:
						...
						// rule #1 : nearby?
						if (!CrystalsController.isNearby(_focusCrystal.id, _crystal.id))
						{
							// refocus
		...
		private function onSwapComplete(crystal:Crystal):void
		{
			...
			if (!_focusCrystal.focus && !_swapCrystal.focus)
			{
				...
				// swap
				CrystalsController.swapByID(_crystals, _focusCrystal.id, _swapCrystal.id);
		...
		private function refill():void
		{
			...
			while (--_index > -1)
			{
				...
				if (_crystal.status == Crystal.STATUS_REMOVED || _crystal.status == Crystal.STATUS_FALL)
				{
					...
					var _aboveCrystal:Crystal = CrystalsController.getAboveCrystal(_crystals, _index, config.COL_SIZE);
					if (_aboveCrystal)
					{
						// fall to bottom
		...
		private function onRefillComplete(crystal:Crystal):void
		{
			...
			if (crystal.swapID != -1)
			{
				CrystalsController.swapPositionByID(_crystals, crystal.id, crystal.swapID);
				CrystalsController.swapByID(_crystals, crystal.id, crystal.swapID);
