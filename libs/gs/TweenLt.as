...
package gs {
	...
	import flash.utils.*;
/**
 * 	TweenLt is a super lightweight (1.6k) version of TweenLite and is only recommended for situations where you absolutely
 *  cannot afford the extra 2.4kb that the normal TweenLite engine would cost. In order to minimize file size, the
 *  following sacrifices have been made in TweenLt (compared to TweenLite):
 * 	<ul>
 * 		<li><b> No Plugins </b>- One of the great things about TweenLite is that you can activate
 * 			plugins in order to add features (like autoAlpha, tint, blurFilter, etc.). TweenLt, however, 
 ...
 * 			tweens can be much easier with TimelineLite and TimelineMax, but TweenLt instances cannot be inserted into
 * 			TimelineLite or TimelineMax instances.
 * 		  
 * 		<li><b> Slight speed decrease </b>- Under very heavy loads, TweenLt won't perform quite as well as TweenLite, but
 ...
 * 			object (overwrite:true or overwrite:false) in TweenLt. TweenLite, however, can use OverwriteManager to expand 
 ...
 * 			for details).
 * 
 * 		<li><b> onStart, easeParams, and defaultEase missing </b>- These features don't exist in TweenLt.
 * 	</ul>
 * 
 * <hr>	
 ...
 * 											
 * 										<li><b> true (ALL):</b> This is the default mode in TweenLt. All tweens of the same object are completely overwritten immediately when the tween is created. <br /><code>
 * 												   		TweenLt.to(mc, 1, {x:100, y:200});<br />
 * 														TweenLt.to(mc, 1, {x:300, delay:2, overwrite:true}); //immediately overwrites the previous tween</code>
 * 										</ul>
 * 	</ul>
 * 
 * <b>EXAMPLES:</b> <br /><br />
 ...
 * 	
 * <code>
 * 		import gs.TweenLt;<br /><br />
 * 		TweenLt.to(mc, 1.5, {alpha:0.5, x:120});
 * 	</code><br /><br />
 *  
 ...
 * 		
 * 	<code>
 * 		import gs.TweenLt;<br />
 * 		import gs.easing.Back;<br /><br />
 * 
 * 		TweenLt.to(mc, 5, {alpha:0.5, x:120, ease:Back.easeOut, delay:2, onComplete:onFinishTween, onCompleteParams:[5, mc]});<br />
 ...
 * 
 * 		TweenLt.from(mc, 5, {y:"-100", ease:Elastic.easeOut});		
 *  </code><br /><br />
 * 
 * <b>NOTES:</b><br /><br />
 * <ul>
 * 	<li> The base TweenLt class adds about 1.6kb to your Flash file.
 * 	  
 * 	<li> Passing values as Strings will make the tween relative to the current value. For example, if you do
 * 	  <code>TweenLt.to(mc, 2, {x:"-20"});</code> it'll move the mc.x to the left 20 pixels which is the same as doing
 * 	  <code>TweenLt.to(mc, 2, {x:mc.x - 20});</code> You could also cast it like: <code>TweenLt.to(mc, 2, {x:String(myVariable)});</code>
 * 	
 * 	<li> Kill all tweens for a particular object anytime with the <code>TweenLt.killTweensOf(mc); </code>
 * 	  function. If you want to have the tweens forced to completion, pass true as the second parameter, 
 * 	  like <code>TweenLt.killTweensOf(mc, true);</code>
 * 	  
 * 	<li> You can kill all delayedCalls to a particular function using <code>TweenLt.killTweensOf(myFunction);</code>
 * 	  This can be helpful if you want to preempt a call.
 * 	  
 * 	<li> Use the <code>TweenLt.from()</code> method to animate things into place. For example, if you have things set up on 
 ...
 */	 
	public class TweenLt {
		...
		/** @private Indicates whether or not the TweenLt class has been initted. **/
		protected static var _tlInitted:Boolean; 
		/** @private **/
		 ...
		 */
		public function TweenLt($target:Object, $duration:Number, $vars:Object) {
			if (!_tlInitted) {			
...
//---- STATIC FUNCTIONS -------------------------------------------------------------------------
		...
		/**
		 * Static method for creating a TweenLt instance which can be more intuitive for some developers 
		 ...
		 * 
		 * var myTween:TweenLt = new TweenLt(mc, 1, {x:100}); <br />
		 * TweenLt.to(mc, 1, {x:100}); <br />
		 * var myTween:TweenLt = TweenLt.to(mc, 1, {x:100});</code>
		 * 
		 * @param $target Target object whose properties this tween affects. This can be ANY object, not just a DisplayObject. 
		 * @param $duration Duration in seconds (or frames if "useFrames" is true)
		 * @param $vars An object containing the end values of the properties you're tweening, like {x:100, y:50}. It can also contain special properties like "onComplete", "ease", "delay", etc.
		 * @return TweenLt instance
		 */
		public static function to($target:Object, $duration:Number, $vars:Object):TweenLt {
			return new TweenLt($target, $duration, $vars);
		}
		
		/**
		 * Static method for creating a TweenLt instance that tweens in the opposite direction
		 * compared to a TweenLt.to() tween. In other words, you define the START values in the 
		 ...
		 * because you can build them in their end positions and do some simple TweenLt.from()
		 ...
		 * 
		 * TweenLt.from(mc, 1.5, {alpha:0, delay:2});</code>
		 * 
		 * @param $target Target object whose properties this tween affects. This can be ANY object, not just a DisplayObject. 
		 * @param $duration Duration in seconds (or frames if "useFrames" is true)
		 ...
		 * @return TweenLt instance
		 */
		public static function from($target:Object, $duration:Number, $vars:Object):TweenLt {
		 ...
		 * 
		 * TweenLt.delayedCall(1, myFunction, ["param1", 2]); <br />
		 ...
		 * @return TweenLt instance
		 */
		public static function delayedCall($delay:Number, $onComplete:Function, $onCompleteParams:Array=null, $useFrames:Boolean=false):TweenLt {
			return new TweenLt($onComplete, 0, {delay:$delay, onComplete:$onComplete, onCompleteParams:$onCompleteParams, useFrames:$useFrames, overwrite:0});
		}
		...
		public static function updateAll($e:Event=null):void {
			_currentFrame++;
			_currentTime = getTimer() * 0.001;
			var ml:Dictionary = _masterList, a:Array, tgt:Object, i:int, t:Number, tween:TweenLt;
			for (tgt in ml) {
				a = ml[tgt];
				i = a.length;
				while (i-- > 0) {
		...
		/**
		 * Kills a particular tween instance, stopping it immediately.
		 *  
		 * @param $tween TweenLt instance to remove
		 */
		public static function removeTween($tween:TweenLt):void {
			if ($tween) {
				$tween.gc = true;
				$tween.active = false;
			}
