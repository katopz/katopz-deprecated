package application.view.components
{
	...
	{
		[Embed(source='../../../assets.swf', symbol='application.view.components.Tile')]
		...
		public function setBoardEnabled(state:Boolean):void
		{
			for ( var i:Number = 0; i < boardSize; i++ )
			{
				...
				{
					var tile:* = this.getChildByName("Tile_" + i + "_" + j) as Tile;
		...
		{
			var tile:* = this.getChildByName("Tile_" + x + "_" + y) as Tile;
		...
		private function onClick(event:MouseEvent):void
		{
			...
			tile.gotoAndStop("userTile");
			setTileEnabled(tile, false);
			
			dispatchEvent( new Event(Board.TILE_CLICK) );
		}
		
		private function setTileEnabled(target:*, state:Boolean):void
		{
			if ( !state )
			{
