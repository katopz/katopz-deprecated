...
package {
	...
	public class Map extends Content{
		...
		public function setMap():void {
			...
			var dataPath = XMLUtil.getNodeById(config,contentParentName+"MapData").@src;
			var method = String(XMLUtil.getNodeById(config,contentParentName+"MapData").@method).toUpperCase();
		...
		public function createMap(xml:XML):void {
			...
			*/
			...
			{
				house.id = house.name;
				house.config = XMLUtil.getNodeById(item, house.id);
				
				if (!house.config) continue;
				...
				if (house.balloon.visible) house.balloon.alpha = 1; else house.balloon.alpha = 0;
				...
				*/
				...
				house.balloon.extra.text = String(house.id);
