...
package com.yahoo.astra.fl.charts
{
	...
	//--------------------------------------
		...
		{				
			...
			if(property != InvalidationType.SIZE && this.parent)
			{
	...
	//--------------------------------------
		...
		protected function drawObjectsOnMajorUnit():void
		{
			...
			for(var i:int = 0; i < labelCount; i++)
			{
				...
				if(i > 0)
				{
					...
					date = this.updateDate(date, this._majorTimeUnit, unitValue, snapToUnits);
				}
				...
				var position:Number = this.valueToLocal(date);
		...
		protected function drawObjectsOnMinorUnit():void
		{
			...
			{	
				...
				date = this.updateDate(date, this.minorTimeUnit, this.minorUnit, snapToUnits);
		...
		/**
		 * Based on an input data set, returns only the points that appear within the
		 * axis range.
		 */
		protected function getVisibleData(data:Array):Array
		{
			...
			var roundedMinimum:Date = DateUtil.roundDown(new Date(minimum.valueOf()), this.majorTimeUnit);
			var roundedMaximum:Date = DateUtil.roundUp(new Date(maximum.valueOf()), this.majorTimeUnit);
			...
			for(var i:int = 0; i < seriesCount; i++)
			{
				...
				for(var j:int = 0; j < seriesLength; j++)
				{
					...
					if(value >= roundedMinimum.valueOf() && value <= roundedMaximum.valueOf())
					{
	...
	//--------------------------------------
		...
		private function setLabelText():void
		{
			...
			for(var i:int = 0; i < labelCount; i++)
			{
				...
				var value:Date = new Date(this.minimum.valueOf());
				var unit:Number = i * this.majorUnit;
				switch(this._majorTimeUnit)
				{
					case TimeUnit.YEAR:
						value.fullYear += unit;
						break;
				...
				if(value.valueOf() != this.minimum.valueOf() && value.valueOf() != this.maximum.valueOf())
				{
					value = DateUtil.roundDown(value, this.majorTimeUnit);
				}
