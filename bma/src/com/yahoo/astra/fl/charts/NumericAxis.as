...
package com.yahoo.astra.fl.charts
{
	...
	//-- Units
		 ...
		 *
		 * @see com.yahoo.charts.ScaleType
		 */
	...
	//--------------------------------------
		...
		override public function valueToLocal(data:Object):Number
		{
			...
			if(this.reverse)
			{
				...
				else position = this._contentBounds.width - position;
			}
		...
		{
			if(property != InvalidationType.SIZE && this.parent is UIComponent)
			{
				(this.parent as UIComponent).invalidate();
			}
		...
		override public function updateScale(data:Array):Array
		{
			...
			return this.getVisibleData(data);
		}
	...
	//--------------------------------------
		...
		protected function resetScale():void
		{
			...
			{
				if(this._minimum > 0 && this._maximum > 0 && !this._minimumSetByUser)
				{
					this._minimum = 0;
				}
				else if(this._minimum < 0 && this._maximum < 0 && !this._maximumSetByUser)
				{
					this._maximum = 0;
				}
			}
			...
			if(!this._minimumSetByUser && !(this.alwaysShowZero && this._minimum == 0))
			{
				...
				if(this.scale == ScaleType.LOGARITHMIC && this._minimum <= 0)
				{
			...
			majorUnitSpacing = lengthOfAxis / ((this._maximum - this._minimum) / calculatedMajorUnit);
			...
			{
				this._minorUnit = Math.min(calculatedMajorUnit / int(majorUnitSpacing / 25), calculatedMajorUnit);
			}
		...
		protected function calculateMaximumLabelBounds(proposedMajorUnit:Number):Point
		{
			...
			var snapToUnits:Boolean = this.getStyleValue("snapToUnits") as Boolean;
			for(var i:Number = this.minimum; i <= this.maximum; i += proposedMajorUnit)
			{
				if(snapToUnits) i = NumberUtil.roundToNearest(i, proposedMajorUnit);
		...
		protected function drawObjectsOnMajorUnit():void
		{
			...
			while(value <= this.maximum)
			{
				...
				var position:Number = this.valueToLocal(value);
				...
				//a bad major unit will get us stuck in an infinite loop, so break.
				if(this.majorUnit <= 0) break;
				
				//if the maximum has been displayed, we're done!
				if(displayedMaximum) break;
				
				value += this.majorUnit;
				if(snapToUnits) value = NumberUtil.roundToNearest(value, this.majorUnit);
					...
					if(this.orientation == AxisOrientation.VERTICAL)
					{
						if(lastVisibleLabel)
						{
								...
								!this.reverse && lastVisibleLabel.y + lastVisibleLabel.height > label.y)
							{
								...
								/*if(labelCount == this.labels.length - 1) lastVisibleLabel.visible = false;
		...
		protected function drawObjectsOnMinorUnit():void
		{
			...
			while(value <= this.maximum)
			{
				...
				while(skipMajorUnits &&
					(NumberUtil.fuzzyEquals((value - this.minimum) % this.majorUnit, 0) || snapToUnits && NumberUtil.fuzzyEquals((value) % this.majorUnit, 0))
					|| NumberUtil.fuzzyEquals(value, this.maximum))
				{
					...
					if(snapToUnits) value = NumberUtil.roundToNearest(value, this.minorUnit);
					value = Math.min(value, this.maximum); //don't go over the max!
					displayedMaximum = NumberUtil.fuzzyEquals(value, this.maximum);
				}
		...
		/**
		 * @private
		 * Determines the set of data that is visible based on the axis scale.
		 */
		protected function getVisibleData(data:Array):Array
		{
			var seriesCount:int = data.length;
			...
			for(var i:int = 0; i < seriesCount; i++)
			{
				var series:ISeries = data[i].clone();
				var dataField:String = this.plotArea.axisAndSeriesToField(this, series);
				var seriesLength:int = series.length;
				var updatedSeriesData:Array = [];
				for(var j:int = 0; j < seriesLength; j++)
				{
					var item:Object = series.dataProvider[j];
					if(item is Number)
					...
					{
						updatedSeriesData.push(item);
					}
				}
				series.dataProvider = updatedSeriesData;
				updatedData.push(series);
			}
			return updatedData;
		}
