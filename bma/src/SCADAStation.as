	...
	public class SCADAStation extends Content
	{
		...
		public function setStation(iID:String, iName:String)
		{
			...
			if (iID == "E00")
			{
				...
				//name
				iE00.iStationName.text = iID + " - " + iName;
			...
			else
			{
				...
				if (station.extra.id != iID)
				{
					...
					popup.iStationName.text = iID + " - " + iName;
				}
				else
				{
					update()
		...
		{
			var dataPath = StringUtil.getNodeById(config, "SCADAStationData").@src;
			var method = String(StringUtil.getNodeById(config, "SCADAStationData").@method).toUpperCase();
		...
		public function createSection(type)
		{
			...
			switch (type)
			{
				...
				default:
					if (currentSectionID == "E23")
					{
						...
						waterGate.waterRight = waterGate.addChild(new Water(new Rectangle(272, -180, 40, 180)));
						waterGate.waterRight2 = waterGate.addChild(new Water(new Rectangle(272 + 40, -180, 40, 180)));
					}
					else
					{
						waterGate.water = waterGate.addChild(new Water(new Rectangle(118, -180, 150, 180)));
			...
			switch (type)
			{
				...
				default:
					...
					if (currentSectionID == "E23")
					{
						waterGate.waterRight2.y = 180
						waterGate.waterRight2.height = y0 + waterObject.right2 * factor;
						trace("#1:" + waterObject.right);
						trace("#2:" + waterObject.right2);
						waterGate.water.height = (waterGate.waterLeft.height + waterGate.waterRight.height + waterGate.waterRight2.height) / 3
					}
					break;
			}
		...
		public function onCompleteAll(xml):void
		{
			...
			for (i = 0; i < maxPack; i++)
			{
				...
				if (i < total)
				{
					...
					pack.label.htmlText = String(item.LABEL);
					pack.label.rotation = 90;
				}
				else
				{
					pack.visible = false;
		...
		public function onComplete(event:Event):void
		{
			...
			else
			{
				...
				section.extra.detail.FLOW.text = item.FLOW

				if (String(item.DOOR_STATUS.DOOR).length > 0)
				{
					section.extra.detail.label.MSL_VALUE_IN.text = item.MSL_VALUE_IN
					section.extra.detail.label.MSL_VALUE_OUT.text = item.MSL_VALUE_OUT
					section.extra.detail.label.visible = true;

					//2009/09/30 special request for E23
					if (String(item.@id) == "E23")
					{
						section.extra.detail.label.MSL_VALUE_OUT2.text = item.MSL_VALUE_OUT2
					}
					else
					{
						section.extra.detail.label.MSL_VALUE_OUT2.text = "";
					}
				}
				else
				{
				...
				try
				{
					if (String(item.@id) == "E41" || String(item.@id) == "E14")
					{
						...
						section.extra.detail.specialBMA.gotoAndStop(1);
					}
					else if (String(item.@id) == "E23")
					{
						section.extra.detail.label.special.gotoAndStop(3);
						section.extra.detail.specialBMA.gotoAndStop(2);
					}
					else
					{
				...
				section.extra.detail.BMA_VALUE_IN.text = item.BMA_VALUE_IN
				section.extra.detail.BMA_VALUE_OUT.text = item.BMA_VALUE_OUT

				if (String(item.@id) == "E23")
				{
					section.extra.detail.BMA_VALUE_OUT2.text = item.BMA_VALUE_OUT2;
					section.extra.detail.BMA_VALUE_OUT.x = 232;
				}
				else
				{
					section.extra.detail.BMA_VALUE_OUT2.text = "";
					section.extra.detail.BMA_VALUE_OUT.x = section.extra.detail.BMA_VALUE_OUT2.x;
				}

				waterObject = new Object();
				waterObject.left = Number(item.MSL_VALUE_IN);
				waterObject.right = Number(item.MSL_VALUE_OUT);
				waterObject.right2 = Number(item.MSL_VALUE_OUT2);
				
				waterObject.label = Number(item.LABEL);
				...
				if (String(item.DOOR_STATUS.DOOR).length > 0)
				{
					for each (var door in item..DOOR_STATUS.DOOR)
					{
						...
						//section.extra.detail["waterValue"+item.DOOR_STATUS.@num+"_"+door.INDEX].text = waterGateObject.height;
