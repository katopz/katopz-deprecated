package
{
	...
	public class ExLevel extends Sprite
	{
		[Embed(source="embed/3.jpg")]
		private const SampleBitmap:Class;
		private const sampleBitmap:Bitmap = new SampleBitmap as Bitmap;

		//-- test environment
		private const videoWidth:int = sampleBitmap.width;
		private const videoHeight:int = sampleBitmap.height;
		private const imageFormat:int = ImageFormat.RGB;
		private const showHistograms:Boolean = true;
		...
		private var _level:LevelsCorrection = new LevelsCorrection(true);
		...
		private var _to:Number = 255;

		private function onWheel(event:MouseEvent):void
		{
			/*
			   _gamma += event.delta/10;
		...
		private function onEnterFrame(event:Event):void
		{
			...
			image.loadBitmapData(sampleBitmap.bitmapData, true);
			...
			if (showHistograms)
			{
				...
				screen.draw(tmp, translate,null,null,null,true);
				tmp.dispose();
			}
			...
			if (showHistograms)
			{
				...
				debugField.appendText('to: ' + _to + '\n');
				debugField.appendText('from: ' + _from + '\n');
				debugField.appendText('Gamma: ' + _gamma + '\n');
