package org.papervision3d.objects.parsers
{
	 ...
	 * @author Tim Knip
	 */ 
	public class DAE extends DisplayObject3D implements IAnimationDataProvider, IAnimatable, IClip3D
	{
		...
		private var currentLoops:Dictionary = new Dictionary(true);
		...
		public function createLabel():void
		{
			...
			trace(" _endTime : " + _endTime);
			trace(" setLabels : " + labels, frames);
			*/
			
			trace(" ! labels : " + document.extras["labels"]);
			trace(" ! frames : " + document.extras["frames"]);
			
			labels = document.extras["labels"];
			frames = document.extras["frames"];
			
			for (var i:int=0; i< labels.length; i++)
			{
				var _frames:Array = String(frames[i]).split("-");
				currentLoops[labels[i]] = 
				{
					beginTime	:getTimeByFrame(_frames[0]), 
					endTime		:getTimeByFrame(_frames[1])
				};
			}
		...
		public function getTimeByFrame(frameNumber:int):Number
		{
			return _endTime*frameNumber/_totalFrames;
		}
		
		public function setLoopByLabel(label:String):void
		{
			//current frame label 
			currentLabel = label;
			...
			if(currentLabel)
			{
				//current loop beginTime -> endTime
				var action:Object = currentLoops[currentLabel];
				if(action && action.beginTime && action.endTime)
				{
		...
		{
			if(!_channels)return super.project(parent, renderSessionData);	
			...
			if(_isPlaying && _channels)
			{
				...
				var elapsed:Number = beginTime + (getTimer()/1000) - secs;
				
				//katopz : fix overlap
				//if(elapsed > duration-.5)
				if(elapsed < beginTime)
				{
					elapsed = beginTime;
				}
		...
		protected function buildAnimationChannels():void
		{
			...
			for(var object:* in channelsByObject)
			{
				...
				// katopz : try other
				if(!transform && channels[1])
					transform = node.findMatrixBySID(channels[1].syntax.targetSID);
		...
		{			
			...
			controller.joints = [];
			controller.vertexWeights = [];
			controller.invBindMatrices = [];
		...
		{	
			...
			_channels = this.getAnimationChannels() || [];	
