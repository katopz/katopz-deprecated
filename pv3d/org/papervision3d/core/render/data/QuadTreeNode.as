			...
			if (level < maxlevel) {
	            if (pri.maxX <= xdiv)
	            {
	                if (pri.maxY <= ydiv)
	                {
	                    if (lefttop == null) {
	                    	lefttopFlag = true;
	                        lefttop = new QuadTreeNode(xdiv - halfwidth*.5, ydiv - halfheight*.5, halfwidth, halfheight, level+1, this, maxlevel);
	                    } else if (!lefttopFlag) {
	                    	lefttopFlag = true;
	                    	lefttop.reset(xdiv - halfwidth*.5, ydiv - halfheight*.5, halfwidth, halfheight, maxlevel);
	                    }
	                    lefttop.push(pri);
	                    return;
	                }
	                else if (pri.minY >= ydiv)
	                {
	                	if (leftbottom == null) {
	                    	leftbottomFlag = true;
	                        leftbottom = new QuadTreeNode(xdiv - halfwidth*.5, ydiv + halfheight*.5, halfwidth, halfheight, level+1, this, maxlevel);
	                    } else if (!leftbottomFlag) {
	                    	leftbottomFlag = true;
	                    	leftbottom.reset(xdiv - halfwidth*.5, ydiv + halfheight*.5, halfwidth, halfheight, maxlevel);
	                    }
	                    leftbottom.push(pri);
	                    return;
	                }
	            ...
	            else if (pri.minX >= xdiv)
	            {
	                if (pri.maxY <= ydiv)
	                {
	                	if (righttop == null) {
	                    	righttopFlag = true;
	                        righttop = new QuadTreeNode(xdiv + halfwidth*.5, ydiv - halfheight*.5, halfwidth, halfheight, level+1, this, maxlevel);
	                    } else if (!righttopFlag) {
	                    	righttopFlag = true;
	                    	righttop.reset(xdiv + halfwidth*.5, ydiv - halfheight*.5, halfwidth, halfheight, maxlevel);
	                    }
	                    righttop.push(pri);
	                    return;
	                }
	                else if (pri.minY >= ydiv)
	                {
	                	if (rightbottom == null) {
	                        ...
	                        rightbottom = new QuadTreeNode(xdiv + halfwidth*.5, ydiv + halfheight*.5, halfwidth, halfheight, level+1, this, maxlevel);
	                    } else if (!rightbottomFlag) {
	                    	rightbottomFlag = true;
	                    	rightbottom.reset(xdiv + halfwidth*.5, ydiv + halfheight*.5, halfwidth, halfheight, maxlevel);
	                    }
