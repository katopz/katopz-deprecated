package org.papervision3d.core.math
{
	...
	public class Quaternion 
	{
		...
		public static function createFromOrthoMatrix( m:Matrix3D ):Quaternion
		{
			...
			q.w = Math.sqrt( Math.max(0, 1 + m.n11 + m.n22 + m.n33) )*.5;
			q.x = Math.sqrt( Math.max(0, 1 + m.n11 - m.n22 - m.n33) )*.5;
			q.y = Math.sqrt( Math.max(0, 1 - m.n11 + m.n22 - m.n33) )*.5;
			q.z = Math.sqrt( Math.max(0, 1 - m.n11 - m.n22 + m.n33) )*.5;
